...
import type { StageScoringWeights, ScoringWeights, InvestorScoringPreference, InvestmentThesis } from "@shared/schema";
...
export default function InvestorScoring() {
  ...
  const [minThesisFit, setMinThesisFit] = useState(0);
  ...
  const { data: thesis, isLoading: isLoadingThesis } = useQuery<InvestmentThesis>({
    queryKey: ["/api/investor/thesis"],
  });

  useEffect(() => {
    if (thesis) {
      setMinThesisFit(thesis.minThesisFitScore || 0);
      setMinStartupScore(thesis.minStartupScore || 0);
    }
  }, [thesis]);

  const thresholdMutation = useMutation({
    mutationFn: async (thresholds: { minThesisFitScore: number; minStartupScore: number }) => {
      return apiRequest("PATCH", "/api/investor/thesis/thresholds", thresholds);
    },
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your matching thresholds have been updated.",
      });
    },
    onError: () => {
      toast({
  ...
  const isLoading = isLoadingWeights || isLoadingPrefs || isLoadingThesis;
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>Minimum Thesis Fit Score</Label>
                <Badge variant="secondary">{minThesisFit}%</Badge>
              </div>
              <Slider
                value={[minThesisFit]}
                onValueChange={([value]) => setMinThesisFit(value)}
                min={0}
                max={100}
                step={5}
                className="w-full"
                data-testid="slider-thesis-fit"
              />
              <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>Minimum Startup Score</Label>
                <Badge variant="secondary">{minStartupScore}%</Badge>
              </div>
              <Slider
                value={[minStartupScore]}
                onValueChange={([value]) => setMinStartupScore(value)}
                min={0}
                max={100}
                step={5}
                className="w-full"
                data-testid="slider-startup-score"
              />
              <p className="text-xs text-muted-foreground">
          ...
          <div className="flex justify-end">
            <Button
              onClick={() => thresholdMutation.mutate({ 
                minThesisFitScore: minThesisFit, 
                minStartupScore 
              })}
