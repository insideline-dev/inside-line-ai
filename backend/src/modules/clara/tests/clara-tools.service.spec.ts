...
describe("ClaraToolsService", () => {
  ...
  describe("buildTools", () => {
    it("returns an object with all expected tool keys", () => {
      const tools = service.buildTools(INVESTOR_ID);

      expect(tools).toHaveProperty("getMyMatches");
      expect(tools).toHaveProperty("getMyPipeline");
      expect(tools).toHaveProperty("getStartupDetails");
      expect(tools).toHaveProperty("getStartupStatus");
      expect(tools).toHaveProperty("getMyThesis");
      expect(tools).toHaveProperty("getMyNotes");
      expect(tools).toHaveProperty("getMyPortfolio");
      expect(tools).toHaveProperty("searchStartups");
    });
    ...
    it("each tool has an execute function", () => {
      ...
      for (const key of Object.keys(tools)) {
        expect(typeof (tools as Record<string, { execute: unknown }>)[key].execute).toBe("function");
      }
    });
  });
