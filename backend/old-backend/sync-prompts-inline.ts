...
export async function syncPromptsToDatabase(): Promise<{ successCount: number; failCount: number; details: string[] }> {
    ...
    try {
      ...
      if (existing.length > 0) {
        await db
          ...
          .where(eq(agentPrompts.agentKey, agentKey));
        const msg = `Updated ${agentKey} (v${existing[0].version} -> v${existing[0].version + 1})`;
        console.log(`✓ ${msg}`);
        details.push(msg);
      } else {
        ...
        const msg = `Created ${agentKey} (v1)`;
        console.log(`✓ ${msg}`);
        details.push(msg);
      }
      successCount++;
    } catch (error) {
      const msg = `Failed to sync ${agentKey}: ${error}`;
      console.error(`✗ ${msg}`);
      details.push(msg);
      failCount++;
    }
