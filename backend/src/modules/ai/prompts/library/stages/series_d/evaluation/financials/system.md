...
- DATA GAPS: What financial information is missing. For each gap, assess:
  - Gap description (SEC-ready reporting for diligence, comprehensive scenario analysis absent, revenue recognition policy unknown)
...
- charts.marginProgression[] → array of { period, grossMargin, operatingMargin }. Expected at Series D+ with public-company-grade models.

Financial Planning Maturity (only populate when financialModelProvided is true):
- financialPlanning.sophisticationLevel → "basic", "developing", "solid", "advanced", or "ipo-grade". At Series D+, expect "advanced" or "ipo-grade".
- financialPlanning.diligenceFlags[] → array of { flag, priority ("critical", "important", "routine") }. At Series D+, include audit readiness, SEC reporting, revenue recognition, financial controls flags.
...
Data Gaps:
- dataGaps[] → array of { gap, impact ("critical", "important", "minor"), suggestedAction }. Include "Financial model not provided" when financialModelProvided is false. At Series D+, a missing model is a critical gap.
