...
import { Upload, Globe, FileText, Building2, MapPin, Briefcase, Loader2, CheckCircle, Users, Plus, Trash2, Linkedin, DollarSign, TrendingUp, Package, Video, Image, User, Mail, Phone, History, Info, Save, Cloud, CloudOff } from "lucide-react";
import type { StartupDraft } from "@shared/schema";
...
type SubmitFormData = z.infer<ReturnType<typeof createSubmitSchema>>;
...
export function StartupSubmitForm({ 
  ...
  const draftIdFromUrl = new URLSearchParams(searchParams).get("draft");
  ...
  const [draftId, setDraftId] = useState<number | null>(draftIdFromUrl ? parseInt(draftIdFromUrl) : null);
  const [draftStatus, setDraftStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");
  const [lastSavedAt, setLastSavedAt] = useState<Date | null>(null);
  const [isLoadingDraft, setIsLoadingDraft] = useState<boolean>(!!draftIdFromUrl);
                      ...
                      <FormItem className="space-y-3">
                        ...
                        <FormControl>
                          <RadioGroup
                            ...
                            data-testid="radio-valuation-type"
                          >
                            <div className="flex items-center space-x-2">
                              <RadioGroupItem value="pre_money" id="pre_money" data-testid="radio-pre-money" />
                              <Label htmlFor="pre_money" className="font-normal cursor-pointer">Pre-money</Label>
                            </div>
                            <div className="flex items-center space-x-2">
                              <RadioGroupItem value="post_money" id="post_money" data-testid="radio-post-money" />
                              <Label htmlFor="post_money" className="font-normal cursor-pointer">Post-money</Label>
                            </div>
                          </RadioGroup>
                        </FormControl>
                        <FormMessage />
