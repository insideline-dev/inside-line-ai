...
export function ProductTabContent({ startup, evaluation, showScores = true, productWeight }: ProductTabContentProps) {
  ...
  const extractedFeatures = (evaluation?.extractedFeatures as unknown as ExtractedFeature[]) || [];
  const extractedTechStack = (evaluation?.extractedTechStack as unknown as ExtractedTech[]) || [];
  const aiExtractedScreenshots = (evaluation?.extractedScreenshots as unknown as { url: string; source: string; caption?: string }[]) || [];
