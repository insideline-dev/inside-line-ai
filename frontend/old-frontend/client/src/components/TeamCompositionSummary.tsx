...
function RoleIndicator({ label, hasRole, icon: Icon }: { 
  label: string; 
  ...
  return (
    <div className={`flex items-center gap-2 p-3 rounded-lg border ${
      hasRole ? "bg-chart-2/10 border-chart-2/30" : "bg-chart-5/10 border-chart-5/30"
    }`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <span className={`text-4xl font-bold ${
                teamScore >= 80 ? "text-green-600" :
                teamScore >= 60 ? "text-amber-600" :
                "text-red-600"
              }`} data-testid="text-team-score">{teamScore}</span>
