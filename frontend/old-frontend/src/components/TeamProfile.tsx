...
export function TeamProfileCard({ member, showTimelines = true }: TeamProfileCardProps) {
        ...
        <div className="p-6 border-b bg-gradient-to-br from-primary/5 to-transparent">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                ...
                {member.fmfScore !== undefined && member.fmfScore !== null && (
                  <Badge className={`${getScoreColor(member.fmfScore)} border-0 shrink-0`}>
          ...
          {member.relevantExperience && (
            <div className="mt-4 p-3 bg-primary/5 rounded-lg">
              <p className="text-sm">
                <span className="font-medium text-primary">Relevant Experience:</span>{" "}
                <span className="text-muted-foreground">{member.relevantExperience}</span>
              </p>
            </div>
          )}
        </div>
      ...
      <div className="relative pl-6 space-y-4">
        ...
        {experiences.map((exp, idx) => (
          <div key={idx} className="relative">
            {/* Timeline dot */}
            <div className={`absolute -left-6 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-background ${
            ...
            <div className="space-y-1">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h5 className="text-sm font-medium">{exp.title || exp.position || "Position"}</h5>
                ...
                {(exp.startDate || exp.start || exp.endDate || exp.end) && (
                  <span className="text-xs text-muted-foreground flex items-center gap-1 shrink-0">
                    ...
                    {exp.startDate || exp.start || "?"} - {exp.isCurrent || (!exp.endDate && !exp.end) ? "Present" : exp.endDate || exp.end || "?"}
                  </span>
                )}
              </div>
              {exp.description && (
        ...
        {education.map((edu, idx) => (
          <div key={idx} className="relative">
            ...
            <div className="space-y-1">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h5 className="text-sm font-medium">{edu.school || "Institution"}</h5>
